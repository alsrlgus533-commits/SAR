...
- 백엔드 엔드포인트: `POST /report/hwpx` — body `{ utterance, center, extra:{경위,피해,조치}, confirmed:{사고일시,선박명,사고위치,항로,출항시각,여객,승무원,사고개요} }`. 응답은 hwpx 바이트(`Content-Disposition: attachment`). 필수 확정값이 빠졌거나 형식이 잘못되면 422와 `missing` 목록을 반환한다. 프론트 ③단계는 자동추출·연계값을 편집 가능한 검토 폼으로 보여주고 누락값 입력 전 다운로드·최종발송을 막는다.
...
- **보고자가 검토한 `confirmed` 값이 자동추출·외부조회보다 최우선**이며, 나머지 제원은 회사 선박마스터 > 회사 실시간 여객수(신선·`/pax`) > KOMSA/MTIS > LLM 추정 순으로 병합한다. 사고개요 필수정보는 생략하거나 `○○` 자리표시자로 대체하지 않으며, 하나라도 없으면 생성을 중단하고 웹 검토 폼 또는 카카오 대화에서 사용자에게 요청한다. `_build_report_data()`는 `_parse_nl`·`_vessel_lookup`·`_route_lookup`·`_predep_lookup`·`_weather_lookup`(기존 재사용) + `_vessel_master` + `_infer_report_fields`를 병합한다. **외부 조회는 `ThreadPoolExecutor`로 병렬 실행**(파싱 후 vessel/route/infer/VMS 동시 → predep·master는 선박코드 의존, weather는 최종 좌표 의존)해 응답시간을 순차 합산→최댓값으로 단축. 신고문에 좌표가 있으면 VMS(Chromium) 호출은 생략
- **공식 사고분류 강제**: `사고종류`는 `_ACCIDENT_TYPE_LABELS` 18종, `사고 추정원인`은 `_ACCIDENT_CAUSE_LABELS` 23종 중 명칭 하나만 출력한다. LLM이 `12번 기관계통 고장으로 판단`처럼 번호·설명을 반환해도 `_normalize_accident_type()`/`_accident_cause()`와 최종 `_build_report_data()` 경계에서 공식 명칭(`기관계통 고장`)으로 재정규화한다. 보고서에는 분류번호를 표시하지 않으며 미분류는 `기타`로 처리한다.
...
### 상대위치 표기 = '가장 가까운 기항지(항구) 기준 ○○항'
- 운영진 확정 규칙: 사고 좌표 → **가장 가까운 여객선 기항지(`_REF_POINTS`)** 를 기점으로, 이름은 **`○○항`**, 거리·방위는 **그 항구 지점에서** 계산. 여객선이 다니는 항구를 기준으로 표기하는 게 보고 관행에 맞다는 판단(예: 흑산항 남서쪽 6마일, 다물도항 동쪽 2마일, 제주항 북동쪽 3마일)
- 런타임: `backend.py`
  - `_port_label(name)`: 기항지명을 `○○항`으로(괄호 안 접안지 세부는 제거: `다물도(다촌)`→`다물도항`, `제주(제주시)`→`제주항`; 이미 `항`/`등대`/`방파제` 포함 이름은 그대로: `부산항북방파제`·`가덕도등대`·`안흥신항`)
  - `_rel_position_detail(lat,lon)`: 최근접 기항지 1개 선정 → `{name(○○항),거리,방위,dir8,text}`. `_rel_position()`은 text 래퍼(기존 호출부 호환)
  - **`GET /relpos?lat=&lon=`**: 위 결과 JSON. 프론트 1차보고서 빌드가 이 엔드포인트를 우선 호출, 실패 시 클라이언트 `relPosition()`로 폴백. `_REF_POINTS`/`refPoints` 점 목록은 불변이라 기존 동기화 검증과 무관
- (검토 후 폐기) **섬 외곽선 기반 거리**: 큰 섬은 항구가 한쪽에 치우쳐 거리가 부푸는 점을 보정하려 '섬 실제 외곽선까지 거리'(국토지리정보원 해안선 shp + OSM 보충 + 섬명 라벨링) 방식을 시범했으나, **운영진이 '가장 가까운 항구(○○항) 기준'을 채택**해 폐기함(관련 스크립트·데이터 미보존)
